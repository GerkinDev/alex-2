...
echo "Define git host"
echo -e "Host *\n\tUser ${GIT_USERNAME}\n\n" >> /root/.ssh/config
fi

if [ ! -z "$SSH_KEY" ]; then
echo "Use git ssh key"
...
# Setup git variables
if [ ! -z "$GIT_EMAIL" ]; then
echo "defining git user email"
git config --global user.email "$GIT_EMAIL"
fi
if [ ! -z "$GIT_NAME" ]; then
echo "defining git user name"
...
if [ ! -d "/var/www/html/.git" ]; then
	echo "Maybe cloning git to our site"
	# Pull down code from git for our site!
	if [ ! -z "$GIT_REPO" ]; then
		echo "Cloning git to our site"
...
echo "Copying nginx-site-ssl.conf"
